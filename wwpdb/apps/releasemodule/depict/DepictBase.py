...
class DepictBase(ModuleBaseClass):
    ...
    def __cleanupEmOnlyEntries(self):
        for dir in self._resultList:
            if (not 'pdb_id' in dir) or (not dir['pdb_id']):
    ...
    def __getCommunicationLockLabel(self, dataDict):
        if (not 'status_code' in dataDict) or (not 'locking' in dataDict) or (str(dataDict['locking']).upper() == 'WFM') or \
    ...
    def __getSelectedDataFromEntryPickle(self, entryPickle):
        ...
        for pickleData in pickleList:
            if (not 'option' in pickleData) or (not pickleData['option']) or (pickleData['option'] == 'pull_release'):
                continue
            #
            selectedData = {}
            ...
            if ('revision' in pickleData) and pickleData['revision']:
                for rDict in pickleData['revision']:
                    if (not 'revision_type' in rDict) or (not rDict['revision_type']):
                        continue
                    #
                ...
                #
            #
            selectedText,selectedOptions = self._getReleaseOptionFromPickle(pickleData)
            if selectedOptions:
