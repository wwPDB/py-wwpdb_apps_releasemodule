...
class UpdateBase(MessageBaseClass):
    ...
    def _openScriptFile(self, scriptfile):
        ...
        f.write('setenv COMP_PATH ' + self._cI.get('SITE_CC_CVS_PATH') + '\n')
        f.write('setenv BINPATH ${RCSBROOT}/bin\n')
        f.write('setenv LOCALBINPATH ' + os.path.join(self._cI.get('SITE_LOCAL_APPS_PATH'), 'bin') + '\n')
        f.write('setenv DICTBINPATH ' + os.path.join(self._cI.get('SITE_PACKAGES_PATH'), 'dict', 'bin') + '\n')
        f.write('#\n')
        return f
